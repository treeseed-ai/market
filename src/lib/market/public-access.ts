...
import { resolveMarketApi, resolveMarketPrincipal } from './store.js';
...
export function loadMarketplaceContext(context: AstroLike): MarketplaceContext {
	return {
		store: resolveMarketApi(context),
...
export async function resolveMarketplaceUserProfile(context: AstroLike, username: string) {
	return await createMarketApiFacade(context).loadUserProfileByUsername(username).catch(() => null);
}
