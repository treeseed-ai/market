...
export async function loadInfrastructureSeedState(input) {
    ...
    return {
        ...
        runs: safeArray(runs).filter((run) => seedRunTouchesTeam(run, input.team)).slice(0, 20),
        approvals,
        error: null,
    };
}
function emptySeedState(error) {
    return {
        teamId: null,
        seedNames: ['treeseed'],
...
function runtimeEnvValue(locals, name) {
...
function selectedSeedEnvironment(url, locals) {
...
function selectedSeedName(url, seedNames) {
...
function seedRunTouchesTeam(run, team) {
    ...
    return actions.some((action) => (action.kind === 'team'
