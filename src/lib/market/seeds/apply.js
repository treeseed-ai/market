...
async function resolveLocalPersistTo(projectRoot, env = process.env) {
	...
	if (existsSync(generatedWranglerConfig)) {
		return resolve(dirname(generatedWranglerConfig), '.wrangler', 'state', 'v3', 'd1');
	}
	return resolve(projectRoot, '.wrangler', 'state', 'v3', 'd1');
}

async function createLocalSeedStore(projectRoot, env = process.env) {
	const db = new NodeSqliteD1Database(await resolveLocalPersistTo(projectRoot, env));
