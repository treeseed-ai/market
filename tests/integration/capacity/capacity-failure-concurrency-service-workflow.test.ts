...
describe('capacity failure and concurrency service matrix', () => {
	it('recovers interruption, fences stale leases, limits cross-project pressure, and preserves human policy', async () => {
		...
		try {
			...
			}).createCapacityWorkdayTreeDxWorkspace = async () => ({ workspaceId: 'matrix-workspace' });
			...
			expect(resumed.assignment, JSON.stringify(resumeEvidence)).toMatchObject({ status: 'leased', projectId: 'matrix-project-a', capacityProviderId: 'matrix-backup-provider', executionProviderId: 'matrix-backup-execution', runnerId: 'resumed-runner' });
