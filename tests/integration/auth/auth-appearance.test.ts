...
describe('anonymous auth appearance', () => {
	it('prefers submitted register appearance over cookies', () => {
		...
		context.values.set(COLOR_SCHEME_COOKIE, 'fern');
		context.values.set(THEME_MODE_COOKIE, 'system');
	...
	it('sets long lived lax cookies for the selected appearance', () => {
		...
		expect(context.set).toHaveBeenCalledWith(COLOR_SCHEME_COOKIE, 'tidepool', {
			path: '/',
			sameSite: 'lax',
			maxAge: 60 * 60 * 24 * 365,
			secure: true,
		});
		expect(context.set).toHaveBeenCalledWith(THEME_MODE_COOKIE, 'system', {
			path: '/',
			sameSite: 'lax',
			maxAge: 60 * 60 * 24 * 365,
			secure: true,
