...
- Use `npx trsd stage --plan --json "message"` before staging. `stage` is a fast staging integration command by default: it performs local preflight, merges/pushes exact package and root staging SHAs, and waits hosted staging gates unless `--ci off`, but it does not run release-candidate unless `--release-candidate hybrid|strict` is provided. Execute with `npx trsd stage --verify-deployed-resources --json "message"` when the staging promotion must force live Railway, Cloudflare, HTTP, database, and runner verification.
