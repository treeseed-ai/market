...
- per-model TreeDX operation and filter permissions
- allowed tools
- branch policy
- question policy
...
Groups and signals are deliberately orthogonal. A signal is an immutable event and causal edge candidate. A group is durable project-local classification and membership metadata used to filter that evidence. A subscription may add `groupScope` after selecting its signal contract; group scope never creates an edge or assignment by itself. Eligibility is the intersection of signal evidence, group scope, activity-profile eligibility, estimate/decision/readiness state, and capacity admission. Agents declare one or more direct group memberships; there is no primary group. Agent class never contributes implicit membership. Content, agents, and people use the same project group identities; cross-project group references are semantic links and never propagate membership across project boundaries. Signal publication snapshots the subject content's explicit project groups at its immutable evidence revision. Evidence without subject groups fails group-scoped matching instead of inheriting an agent-selected fallback.

Group coordination policy owns proposal types, eligible activity profiles, participation bounds, human and agent roles, review quorum, handoffs, and allocation defaults. It does not repeat models, events, signal contracts, or subscriptions. When several groups match one signal, coordination resolves explicit scope first, then the most-specific shared direct or effective group. Equal-specificity ambiguity blocks rather than choosing lexically.
...
Agent context definitions are immutable revision references, not inline provider hints. `agent_context_query` entries compile to bounded TreeDX requests; `agent_context_query_set` entries compose exact member revisions; `agent_test` entries execute either definition without starting an agent and assert semantic identities and relations plus item, token, and latency bounds. The API stores status, statistics, assertions, result digest, expiry, and the tested TreeDX snapshot ref; it does not store the returned query content. Readiness distinguishes unchecked, passing, failing, and stale definitions, and only a fresh passing exact definition revision is selectable. An unrelated repository commit does not redefine the query; its commit is result provenance rather than query identity. The local operations runner periodically reruns initialized tests after expiry and uses the expired check identity as deterministic idempotency authority. Workday preflight and demand admission repeat the readiness check and freeze only query/test/check provenance into the assignment. The provider then reads the exact query revision and executes it against TreeDX in real time through assignment-scoped authority, recording the live result ref, digest, and stats. A missing, unchecked, failed, expired, or revision-mismatched check blocks admission; a cached test result can never become agent context.
...
A `ProviderAvailabilitySession` records one provider's reported availability and supply for a bounded period. Execution-provider supply may include a minimum viable assignment duration. This is a scheduling constraint, not provider-owned authority: plans can reject or accommodate it, API selection fails closed when a demand window is too short, and accepted assignments freeze the evaluated minimum deadline. Business-day declarations carry their calendar explicitly so a five-business-day human queue is never treated as five elapsed days.
...
Provider-local constraints include an exact disk-capacity observation. An unavailable observation makes every execution provider unavailable for admission and prevents the provider manager from polling new assignments; it does not close existing evidence or silently abandon a lease. Provider health is degraded under the same condition.
...
`discussion`, `discussion-message`, and `discussion-event` are Git-backed Astro content models. A user message is committed and authoritatively read back through TreeDX before an invocation is eligible. The content commit SHA and message path freeze the shared snapshot used by every mentioned agent. Generic lifecycle events never synthesize conversational messages, and transient token deltas are never persisted. A successful communication assignment requires one durable agent-authored final response.
...
`AgentInvocationRequest` is the single admission abstraction for planning and communication. It records exact agent/profile revision, execution and trigger kinds, priority class, opaque source refs, explicit parent containment, handoff ancestry, admission, assignment, and final-message refs. Pending messages may coalesce only before admission; admitted assignments freeze their source refs. The same Discussion/agent pair is serialized, while distinct mentioned agents may run concurrently. A required human response terminalizes the prior demand and lease; the reply creates a new provenance-linked invocation and assignment rather than reopening custody.

Provider availability metadata includes the reconciled runtime source-closure digest. A conversation execution freezes the exact advertised digest alongside its message, definition, and provider provenance. Runtime readiness and guarantee proof compare this observed generation with the managed desired image closure; availability freshness alone cannot qualify a stale container.

Provider lanes have semantic purpose. Communication requires a provider with at least two global slots and distinct communication and operation lanes. Communication may overflow to idle operation capacity with explicit evidence and higher queue priority, but cannot preempt running operation work. Operation work is categorically denied from communication lanes.

Every assignment carries `treeseed.capacity-budget/v2` with separate preparation, productive execution, protected closeout, and total-custody timing. `requestedSeconds`, `executionSeconds`, provider minimum-duration evaluation, and execution utilization describe productive work only. Preparation has its own start/deadline and may perform admitted dynamic context retrieval plus the mandatory initial operational plan. Exact TreeDX plan read-back starts the full execution clock through an idempotent API transition. Execution and closeout have independent deadlines; closeout is restricted to preservation, validation, checkpoint, summary, discussion, and continuation work. The assignment status operation computes authoritative wall-clock `now`, current phase, phase deadline, phase-specific remaining seconds, and `shouldCloseOut`; prompt snapshots are advisory and never replace this live read. Terminal disposition is one of `completed`, `completed_early`, `deadline_exhausted`, `budget_exhausted`, `blocked`, `cancelled`, or `failed`. `completed_early` additionally proves passed acceptance checks, durable artifact refs, remaining budget, a completion reason, and `noUsefulScopedWorkRemaining: true`.

Execution mode is independent from environment and kernel safety mode. `simulation` and `production` freeze into the workday, graph, demand, assignment, TreeDX handle, manifest, mutation receipt, signal, performance record, and guarantee evidence. Both modes run the real provider and lifecycle. Simulation authority is always `denied` for upstream mutation and can integrate only into an isolated local target. A production assignment has `checkpoint-only` authority; only the reviewed platform integration receipt may carry `exact-approved-ref`, and `trsd save` rejects simulation or mixed-mode evidence.

Project-to-project agent deployment creates governed local forks, never shared mutable definitions. Selection is exactly one agent or one effective group at an exact source ref. The deployment closure includes only reachable groups, queries, query tests, templates, topics, signals, proposal types, and agent tests. Target-specific identities and repository scopes require explicit typed bindings. All copied agents begin dormant; activation requires exact-ref target query, reference, permission, signal, provider, and forbidden-authority evidence. Upgrade is an explicit three-way comparison between the last imported source, requested source, and current target, and local divergence fails closed.
