...
`trsd capacity checkpoint-integrate --execute` writes governance execution authority to workset-local `.treeseed` state after validating the API-selected decision graph, assignment, deliverable, review, verification, repository, base commit, checkpoint, and path evidence. Agent checkpoint commits carry a `Treeseed-Assignment` trailer. Save refuses a trailer without its matching authority and refuses later changes to authority-covered paths. That receipt does not push or stage anything. Save turns it into candidate provenance only after the repository save and fresh remote proof succeed. A commit receipt proves what exists; the decision chain proves why that work was allowed.
...
The target Platform workset is reconstructed from the authenticated live team project inventory with `trsd platform workset --plan|--apply`; repository refs are observed live and frozen in the workset receipt. No checked-in file is team-membership authority. The workset receipt under `.treeseed/worksets/platform/` proves isolated repository custody only; the integration change-set receipt under `.treeseed/workflow/integration-receipts/` proves a pushed candidate. Neither receipt substitutes for the other.
...
Stage requires a clean pushed task branch. When the integration receipt contains governed execution, stage fails closed unless the resolved Admin control plane confirms that every decision remains accepted, its proposal revision/hash is current, and its snapshotted cross-project decisions remain current. Validation occurs during planning/preflight and again after merge-down/save before candidate creation. It merges current staging down before promotion, resolves generated internal-reference and lock metadata conflicts, and stops with file-level diagnostics for source conflicts. Package refs are promoted with expected-head protection and the Market staging ref moves last.
