...
The Discussion API commits a user turn through TreeDX before scheduling and selects only the mentioned agents' `chat` activity profiles from that immutable ref. The SDK-owned `discussion-v1` foundation supplies TreeDX reads, bounded message/note/question/proposal writes, common stewardship guidance, and multidimensional budget ceilings. Each project agent declares response style, prompt task, context/tools, functional capability requirements, timebox, tokens, and cost. Provider identity and fallback order belong only to team capacity supply policy.
