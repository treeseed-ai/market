...
Chat assignments use the same kernel and provider execution path but a strictly narrower SDK invariant. Preparation performs dynamic context, bounded Discussion reads, and the initial operational plan before productive time. Productive tools may read/follow/respond, create permitted linked proposal/question/note artifacts, request bounded agent handoffs, prepare an approval-backed operation handoff, or request a semantic client action. They cannot mutate knowledge/source content, decisions, configuration, infrastructure, publication, or another assignment's records. Success requires one durable final Discussion response, terminal summary, usage, settlement, released lease, and zero residue. A required human response checkpoints and closes the assignment; a later reply creates a new invocation and assignment rather than reopening the old lease.

Agent tool availability is fail-closed. `tools.allowed` is the source of truth for execution-provider callable tools, but a listed tool is exposed only when its runtime requirements are present. TreeDX tools require a scoped proxy handle; model-aware content tools also require matching activity-profile `permissions.models` operations and filters; worktree tools require a prepared assignment worktree; SDK dispatch tools require a dispatch-capable SDK context. Missing requirements omit the tool from the callable catalog and are recorded in `agentToolCatalog.omitted` metadata. MCP calls validate tool input before execution and emit assignment-scoped tool-call telemetry.
...
Content permission is separate from provider tool exposure. The selected activity profile's permission policy defines per-model TreeDX operations and filters, workspace commit authority, repository paths and mutation authority, network domains, and shell commands. Handlers cannot exceed the same policy. Execution providers receive only the intersection of `tools.allowed`, the frozen profile permissions, and assignment runtime handles.
...
The provider runner enforces provider-local runtime limits as well, such as concurrency, model availability, local queue pressure, native subscription constraints, and protected host-disk capacity. The provider manager observes disk capacity before polling assignments, advertises exhausted pressure when the reserve plus per-assignment headroom is unavailable, and does not acquire a lease. The runner repeats the check before claiming a manager-created dispatch so a capacity drop between cycles also fails closed.
...
Every resolved activity profile receives the same resource-stewardship instructions. The allocation is a maximum, not a consumption target. An agent finishes as soon as its acceptance criteria are satisfied and no important in-scope work remains; it must not invent work or prolong discussion to consume time or tokens. Before `completed_early`, it verifies outputs, persists required artifacts, reports the remaining budget and reason, and asserts `noUsefulScopedWorkRemaining`. Missing authority, evidence, credentials, or dependencies is `blocked`, not successful early completion.

Every assignment exposes `treeseed.status`, regardless of project-configured tools. System instructions describe three authoritative windows and require a live status call during preparation, after every major phase, before long operations or mutations, and before checkpointing. Preparation admits current dynamic context and the initial durable assignment plan; successful TreeDX plan read-back starts the full productive execution allocation. When execution ends and `shouldCloseOut` becomes true, the agent stops scope expansion, preserves only coherent Zod-valid content or verified source work, creates the applicable immutable commit/checkpoint receipt, and reports exact completed scope, remaining scope, blockers, and resume instructions. Closeout has a separate hard deadline and cannot be consumed by ordinary execution. If project policy grants proposal creation, the agent may submit an ordinary Zod-valid project proposal requesting additional capacity. That proposal neither extends the current assignment nor approves itself.

The kernel receives preparation, productive execution, closeout, and total-custody timing; the provider's evaluated minimum productive duration; phase-specific remaining time; all token classes and ceilings; cost/currency; provider-native caps; concurrency; attempts; pricing generation; and enforcement confidence. Lease renewal changes none of these grants. Terminal status and usage are passed to the API for exactly-once settlement and immediate release of unused reservations, with preparation, productive execution, and closeout reported separately.
