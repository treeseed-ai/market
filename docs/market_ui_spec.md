...
The integrated web tenant currently consists of unchanged Core public/content routes plus Admin's authentication, account, team, active-team, invitation, and public identity/team profile routes. The exact active surface is generated in [ui-routes.md](./ui-routes.md).
...
No deleted route receives a compatibility redirect. Core's homepage owns `/`, public user/team profiles are identity-only, and the root tenant form capability is disabled.
