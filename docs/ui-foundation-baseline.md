...
This is the Phase 1 baseline for the reusable UI foundation in `@treeseed/ui`.
...
- `AuthShell`, `PublicShell`, and `ProductShell` accept `helpContext` and `feedbackContext` without requiring route-local help or feedback UI. Phase 6 contextual help and Phase 5 feedback are shell-level shared components.
...
- The Phase 1 foundation is Astro-first and static by default. Templates, cards, action bars, and permission boundaries must not hydrate client JavaScript.
...
- `TreeseedAppLayout` composes the current `ProductShell` and passes shell-level help/feedback context through.
...
Phase 10 proves the distribution loop on `/app/knowledge`, app knowledge drilldowns, seller readiness, and public marketplace acquisition routes.
...
- Public marketplace acquisition routes under `/market/knowledge-packs/*` and `/market/templates/*` use `TreeseedPublicLayout`, canonical templates, shell help/feedback, and entitlement-aware action summaries.
...
Phase 6 proves contextual help on `/knowledge/*` and `/app/work/questions`.

- Public Knowledge Hub help is resolved by the Core runtime reader view model from the current runtime/local document and navigation metadata, then passed through `MainLayout` to `PublicShell`.
- Product questions help is resolved by the admin questions view-model mapper from capability metadata, resource schema, route template, and resolved action states, then passed through `TreeseedAppLayout` to `ProductShell`.
