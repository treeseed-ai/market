...
| Market API | Planned private commerce extension and Admin-compatible gateway | `treeseed-ai/market-api` is declared but not yet created. It will proxy the complete Admin API baseline and own `/v1/market/**`, catalog, vendor, checkout, Stripe, orders, subscriptions, refunds, fulfillment, and central Market policy. |
