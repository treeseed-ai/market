...
- Availability-session create/refresh/close routes record availability, execution providers, native limits, capabilities, runner pressure, provider minimum viable assignment durations, and provider-local constraints. Provider minimums are SDK-validated scheduling facts; they cannot widen a grant or extend an assignment. Provider-asserted grants are not accepted.
...
- Provider manager and runner admission fail closed on protected host-disk capacity. Availability publishes exact reserve/headroom diagnostics, manager cycles do not lease below the boundary, runner cycles do not claim dispatches below it, and health reports degradation. User-session agent processes also run with core dumps disabled so a native execution-provider crash cannot consume the filesystem before TreeSeed can reconcile cleanup.
...
Discussion sessions, messages, and append-only execution events are Astro content models in project Git and are read or committed only through TreeDX. PostgreSQL stores generic invocation, execution, assignment, lease, lane, handoff, approval, client-action, usage, and settlement records plus opaque Discussion references; it never stores message bodies. Mentioned agents enter the canonical invocation queue only after authoritative TreeDX read-back. Standalone requests create hidden `conversation` executions, while explicit active parent workdays retain exact containment. Chat cannot use `intent: act`; it can only prepare an approval-backed operation handoff that later enters the existing proposal, estimate, plan, and operation-assignment lifecycle.

An approved operation handoff is not acting authority. The demand compiler may
bind it only to an exact work-unit or graph-node identity in an accepted,
scheduled capacity plan backed by the same proposal and accepted decision. The
resulting assignment freezes the handoff and source-message references, runs on
an operation lane, and advances the handoff through scheduled, running, and a
truthful terminal state. Retry recovery clears only the prior assignment binding
and reuses the same approved handoff; it never reopens a lease or creates a
parallel scheduling path. Discussion projection remains append-only lifecycle
event content and never fabricates an agent-authored response.

Every generated two-slot provider declares one `communication` and one `operation` lane. Communication selects its reserved lane first and may overflow to an idle operation lane without preemption; operation demand can never consume communication capacity. Providers below two global slots are visibly not communication-ready. Provider managers long-poll independently per lane slot, and assignment provenance freezes the lane purpose, overflow decision, execution/trigger kind, source messages, invocation, parent, and handoff chain.

Capacity-budget v2 is required at admission for every activity profile. Admission freezes bounded preparation and closeout durations, the provider-compatible productive execution allocation, and a total custody ceiling. Exact initial-plan read-back starts the full execution clock; an explicit or deadline-driven closeout transition ends productive authority and protects critical checkpoint, summary, and cleanup work. Lease renewal cannot extend any grant. Every assignment exposes authoritative live phase status and receives mandatory resumable-closeout instructions. Additional capacity remains a project proposal and subsequent governed assignment, never an in-place self-extension. Preparation, productive execution, closeout, aggregate token ceilings with input/cached/reasoning/output actuals, cost/currency, native units, concurrency, and attempts remain independent dimensions. Completion releases unused claims exactly once. Calibration may recommend p50/p90 time, token, and cost estimates by task signature and provider/model, but it cannot widen team hard limits.
